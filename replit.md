...
- **💾 Save Database (rewired)**: Click the floppy icon on the desktop → builds payload via `buildSavePayload()` (with browser geolocation if permission granted), stashes it in `localStorage.__pending_save_payload`, opens `/save_data.html` in a new tab. The save page shows a confirm dialog ("Press OK to save…"); on OK it POSTs to `/api/save-storage` then redirects to `/saved.html?...summary` (success page with summary tiles + OpenStreetMap iframe if lat/lon present). Captures: full `windowsUserData`, `vmSpecs`, `discord_user_data`, geolocation, full `localStorage` dump, server-resolved IP, IP-API geolocation, user-agent. `storage.json` is append-only (`records[]`).
- **⏱️ Auto-save (every 5s)**: `autoSaveTick()` runs on a 5s interval (configurable via `localStorage.autoSaveInterval`), kicks off 3s after first load. Status pill `#autosave-status` lives in the system tray (clickable to toggle). States: Saving → Saved ✓ → Idle. Persisted via `localStorage.autoSaveEnabled`. CSS pulse animation `@keyframes asPulse` while saving.
- **💬 Discord upgrades**: `DISCORD_CHANNEL_MSGS` per-channel message banks, `discordSwitchChannel()` re-renders messages per channel + updates header `#discord-channel-name` (general/random/memes/announcements/dev-talk + voice channels), unread badges on memes(3)/dev-talk(1), emoji picker (`discordToggleEmojiPicker()` + `DISCORD_EMOJIS` array of 40+ emojis), typing indicator (`discordShowTyping`/`discordClearTyping`) shown briefly before bot replies, `discordSearch()` modal that grep-searches all channel messages, expanded reply pool (12 phrases) and bot pool (Alice/Bob/Carol/Dave/Eve). Channel hover/active styles via `.discord-ch:hover`/`.discord-ch.active`.
- **📷 Camera app** (new): uses `navigator.mediaDevices.getUserMedia`, switch front/back camera, snap photo to canvas, saved photo strip, download as PNG. Graceful fallback when permission denied.
- **▦ QR Code Generator** (new): renders QR via `api.qrserver.com` (proxied through `/proxy?url=`), size + color picker, download + copy text.
- **🐍 Snake game** (new): classic canvas snake, arrow keys/WASD, score + persisted high score in localStorage, game-over notification.
- **🌐 Proxy reliability fix**: `/proxy?url=` now sniffs Content-Type from URL extension (forces `text/css`/`application/javascript` etc. when upstream lies), forwards original `Accept` and a sane `Referer`, sets `X-Content-Type-Options: nosniff`, and the in-page proxy script now uses a `MutationObserver` to rewrite dynamically-injected `<link>`/`<script>`/`<img>`/`<iframe>`/`<source>`/`<video>`/`<audio>` (and `srcset`). `fetch()`/XHR shim now also handles relative URLs against the page's `<base>`.
