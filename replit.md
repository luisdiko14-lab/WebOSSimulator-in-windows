...
- **💾 Save Database button** (new desktop icon): Click the floppy-disk icon → animated overlay with progress bar + live log → POST `/api/save-storage` writes to `storage.json` (root). Captures: full `windowsUserData` (username, email, PIN, security Q/A, theme, accent, timezone, Cortana, find-device, all `privacy*` toggles), `vmSpecs`, `discord_user_data`, browser geolocation (with permission), full `localStorage` dump, plus server-resolved IP, IP-API geolocation (city/region/country/ISP), user-agent, accept-language. MAC: browsers do not expose user MAC for privacy — server logs its own NIC MACs as a reference. `storage.json` is append-only (`records[]` array, with `lastSavedAt`/`totalRecords`).
